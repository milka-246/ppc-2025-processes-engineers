...
|-------------|-------|---------|---------|------------|
| seq | 1 | 6.267 | 1.00 | N/A |
| seq | 1 | 6.346 | 1.00 | N/A |
| mpi | 4 | 1.271 | 4.93 | 123.2% |
| mpi | 4 | 1.750 | 3.58 | 89.5% |
...
1.  **SEQ vs MPI**: MPI реализация показывает значительное ускорение
    ...
    -   task_run: 4.93× ускорение на 4 процессах
        
    -   pipeline: 3.58× ускорение на 4 процессах
        
2.  **Эффективность**:
    
    -   MPI task_run: 123.2% (суперлинейное ускорение)
        
    -   MPI pipeline: 89.5% (высокая эффективность)
        
3.  **Сравнение режимов**:
    
    -   task_run быстрее pipeline на 27.4% в MPI реализации
        
    -   В SEQ реализации разница минимальна (~1%)

**Обсуждение**: 
 
Суперлинейное ускорение (123.2%) в MPI task_run объясняется:

-   Лучшей локализацией данных (кэш-эффект)
    
-   Уменьшением конфликтов доступа к памяти
    
-   Параллельным чтением данных из ОЗУ
    
-   Оптимизациями компилятора GCC 15.2.0
    

**Ограничения**:

-   Каждый процесс хранит полную матрицу B (8 MB)
    
-   Коммуникационные затраты на рассылку B всем процессам
    
-   Дисбаланс нагрузки при некратном распределении строк
...
1.  **Корректность**: 15/15 функциональных тестов пройдены
    
2.  **Производительность**: Достигнуто ускорение 4.93× на 4 процессах
    
3.  **Эффективность**: Суперлинейное ускорение 123.2% в task_run режиме
    
4.  **Масштабируемость**: Хорошая для матриц среднего и большого размера
