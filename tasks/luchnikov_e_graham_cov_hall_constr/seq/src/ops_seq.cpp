...
LuchnikovEGrahamConvexHullSEQ::LuchnikovEGrahamConvexHullSEQ(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType{};
}

bool LuchnikovEGrahamConvexHullSEQ::ValidationImpl() {
  return GetInput().size() >= 3;
}

bool LuchnikovEGrahamConvexHullSEQ::PreProcessingImpl() {
  return true;
}
...
bool LuchnikovEGrahamConvexHullSEQ::PostProcessingImpl() {
  return true;
}
