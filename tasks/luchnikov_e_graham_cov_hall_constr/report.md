...
Особенность реализации — создание как последовательной (SEQ), так и параллельной (MPI) версии с использованием бинарного дерева для слияния частичных результатов. Частные результаты получаются путём разбития точек между процессами (минимум 10 для каждого), после чего каждый процесс строит свои оболочки, а в конце в процессе 0 они соединяются воедино. Цель работы — сравнение производительности двух подходов.
...
```cpp
bool LuchnikovEGrahamConvexHullSEQ::RunImpl() {
...
- **Hardware/OS:** 
 * Процессор: 13th Gen Intel(R) Core(TM) i5-12500Hl
...
- Программа показала значительное ускорение (на 2 процессах: ускорение 1.58×, на 4 процессах: ускорение 2.4×) и хорошую эфективность (на 2 процессах: 79%, на 4 процессах: 60%)
