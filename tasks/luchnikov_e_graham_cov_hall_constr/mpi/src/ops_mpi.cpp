...
LuchnikovEGrahamConvexHullMPI::LuchnikovEGrahamConvexHullMPI(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType{};
}

bool LuchnikovEGrahamConvexHullMPI::ValidationImpl() {
  return GetInput().size() >= 3;
}

bool LuchnikovEGrahamConvexHullMPI::PreProcessingImpl() {
  return true;
}

std::vector<Point> LuchnikovEGrahamConvexHullMPI::GrahamScan(const std::vector<Point> &input_points) {
...
int LuchnikovEGrahamConvexHullMPI::CalculateOptimalActiveProcs(int points_count, int world_size) {
...
bool LuchnikovEGrahamConvexHullMPI::RunImpl() {
  int world_size = 0;
  int world_rank = 0;
...
bool LuchnikovEGrahamConvexHullMPI::PostProcessingImpl() {
  return true;
}
