...
bool OlesnitskiyVDijkstraCrsMPI::ValidationImpl() {
  ...
  if (source < 0 || source >= vertices) return false;
  if (edges.size() != weights.size()) return false;
...
bool OlesnitskiyVDijkstraCrsMPI::RunImpl() {
  ...
  while (active > 0) {
    ...
    MPI_Alltoallv(send_data.data(), send_counts_bytes.data(), send_displs_bytes.data(),
                  MPI_INT, recv_data.data(), recv_counts_bytes.data(), recv_displs_bytes.data(),
  ...
  if (rank == 0) {
    ...
    for (int src = 1; src < size; ++src) {
      MPI_Recv(global_distances.data() + displs[src], counts[src], MPI_INT,
