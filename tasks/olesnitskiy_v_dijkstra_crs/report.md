...
|-------------|-------|---------|---------|------------|
| seq | 1 | 0.5891 | 1.00 | N/A |
| seq | 1 | 0.5823 | 1.00 | N/A |
| mpi | 4 | 3.2640 | 4.93 | 4.5% |
| mpi | 4 | 3.2757 | 3.58 | 4.4% |



**Ключевые метрики**:

-   **Сравнение SEQ и MPI**: Последовательная реализация оказалась значительно быстрее MPI версии
    
-   **Ускорение MPI**: MPI версия работает в ~5.4 раза медленнее SEQ версии
    
-   **Эффективность MPI**: Очень низкая (4.4-4.5%) для 4 процессов
    

**Анализ результатов**:

1.  **Неожиданный результат**: MPI реализация значительно медленнее SEQ версии
    
    -   SEQ pipeline: 0.5891 с
        
    -   MPI pipeline: 3.2640 с (в 5.54 раза медленнее)
        
    -   SEQ task_run: 0.5823 с
        
    -   MPI task_run: 3.2757 с (в 5.62 раза медленнее)
        
2.  **Факторы, влияющие на производительность**:
    
    a) **Коммуникационные затраты**:
    
    -   MPI реализация использует `MPI_Allreduce`, `MPI_Bcast`, `MPI_Alltoallv` на каждой итерации
        
    -   Для графа с 1,000,000 вершин это создает огромные накладные расходы
        
    
    b) **Малая локальность вычислений**:
    
    -   Каждое обновление расстояния требует коммуникации между процессами
        
    -   При равномерном распределении вершин, большинство соседей оказываются на других процессах
        
    
    c) **Синхронизация**:
    
    -   Глобальная координация через `MPI_Allreduce` на каждой итерации
        
    -   Все процессы должны синхронизироваться для выбора следующей вершины
        
3.  **Оптимизация SEQ версии**:
    
    -   Использует эффективную двоичную кучу (`std::priority_queue`)
        
    -   Минимальные накладные расходы на управление памятью
        
    -   Хорошая локальность данных (все данные в одном адресном пространстве)
        

**Обсуждение неэффективности MPI**:

1.  **Характер задачи**: Алгоритм Дейкстры является по своей природе последовательным - на каждой итерации обрабатывается только одна вершина с минимальным расстоянием
    
2.  **Распределение данных**: Для эффективного распараллеливания нужно, чтобы большинство рёбер каждой вершины оставались на том же процессе
    
3.  **Размер задачи**: Для графов с высокой степенью вершин (~100) количество обновлений и коммуникаций становится чрезмерным

## 8. Conclusions

**Основные результаты**:

1.  **Корректность**: Все 14 функциональных тестов пройдены успешно
    
2.  **Производительность SEQ**: Отличная - менее 0.6 секунд для графа с 1M вершин
    
3.  **Производительность MPI**: Неудовлетворительная - в 5.5 раза медленнее SEQ версии
    
4.  **Эффективность MPI**: Очень низкая - 4.4-4.5% на 4 процессах
    

**Анализ причин низкой производительности MPI**:

1.  **Неудачная схема распараллеливания**: Текущая реализация требует слишком много коммуникаций
    
2.  **Высокая степень графа**: При ~100 рёбер на вершину, большинство обновлений требуют межпроцессорной коммуникации
    
3.  **Строгая синхронизация**: Глобальная координация на каждой итерации создает узкое место

**Заключение**:  
Последовательная реализация алгоритма Дейкстры с использованием CRS-формата демонстрирует отличную производительность и эффективность использования памяти. Параллельная MPI реализация требует значительной доработки алгоритмической схемы для достижения приемлемого ускорения на современных многопроцессорных системах.
